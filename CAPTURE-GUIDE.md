...
| `09-01-feuille-de-plans.png` | Une feuille de plans réelle **en milieu de production**, pas vide. | La colonne `status` contienne visiblement des valeurs différentes (`todo`, `image_ok`, `approved`). C'est l'image du tableau de bord. **Si l'atelier dispose d'un outil maison de suivi des plans, c'est ici qu'il se montre** plutôt qu'un tableur. |
...
| **ComfyQ Discovery** | L'application de découverte : quelles machines tournent, quel GPU, quel flux est servi, quels travaux en attente — et un bouton qui ouvre la page de réservation. C'est **par là que les étudiants entrent dans ComfyQ**. Lecture seule. | Son bouton de copie fournit aussi le lien `/admin` de chaque rig. Ce n'est pas un secret — les actions destructrices sont derrière le mot de passe admin — mais **configurez ce mot de passe** avant de déployer en classe : sans lui, ComfyQ refuse toute action inter-utilisateurs, ce qui est le comportement sûr mais bloque aussi votre propre ménage. |
...
- **Les deux liens de téléchargement sont en dur dans le chapitre 3.** Le client LlmOnLan pointe sur une **version épinglée** (`releases/tag/v0.1.25`) et non sur `/releases`, parce que la page `/releases` du dépôt liste d'abord les préversions de la ferme (`farm-v0.0.x`) — un étudiant y prendrait le mauvais fichier. Ce lien épinglé est donc volontaire, mais **il vieillit** : à chaque version majeure du client, mettez-le à jour (deux occurrences dans le manuel, une dans la FAQ). `releases/latest` fonctionnerait aussi, tant que les builds de ferme restent marqués *prerelease*.
